"""Scaling of training performance and effective dimension with MaxFreq (UNbiased data generation).

Scans the control parameter MaxFreq (`max_frequency`, over the values in `max_frequency_vec`,
e.g. [4, 6, 8]) at fixed number of trainable parameters `no_params`. For a partially tensorized
partial Fourier series model f_theta(x) = sum_mu c_mu(theta) e_mu(x) with c_mu(theta) =
sum_nu Gamma_{mu,nu} iota_nu(theta), Gamma = U @ diag(S) @ V^T (dense U on the input side, V^T a
tensor train of bond dimension `bond_dim` on the parameter side).

The data-generating function `Fourier_model_data` is UNBIASED with respect to the full and cutoff
models trained below: it is built from its own independently drawn random orthogonal matrix
`Udata`, an all-ones correlation spectrum `Svals_data`, and an independently drawn random
parameter-side tensor train `Vdata_tensors_jnp` -- none of these are shared with the `U`/`V_tensors`
used by the full/cutoff models. Hence the target y(x) is generic/unrelated to the model class and
is not guaranteed to lie in the expressible function space of either model for any parameter
configuration theta0.

For each of `no_rand_model_tests` random draws of (U, V) at a given MaxFreq value, this script
builds a "full" model (correlation spectrum Svals_full = all ones, i.e. all D singular values
comparable -> high effective dimension) and a "cutoff" model (Svals_cut equal to Svals_full up to
index `cutoff`, then decayed as exp(-decay_exp * i) beyond it -> lower effective dimension),
computes the normalized FIM spectrum and normalized effective dimension (Eq. 12 of the paper) of
both models via Monte-Carlo sampling of `no_params_samples` random parameter points, trains both
models against MSE for `no_train_tests` random parameter initializations, and pickles
...
Other key control parameters (see 'Experiments specs' section below):
    no_rand_model_tests -- number of random (U, V, data-gen) model draws per MaxFreq value
    ...
    no_params            -- fixed number of trainable parameters M (held constant across the MaxFreq scan)
...
no_training_data_per_feature = 50
...
### Vector of maximal Fourier frequencies
max_frequency_vec = [4, 6, 8]
#max_frequency_vec = [10, 12, 14]
#max_frequency_vec = [16, 18, 20]
#max_frequency_vec = [24, 28, 32]
#max_frequency_vec = [36, 40]
#max_frequency_vec = [44, 48]
#max_frequency_vec = [54, 60]
...
for max_frequency in max_frequency_vec:
    ...
    for nm in range(no_rand_model_tests):
        ...
        ### Define random U matrix for data gen.
        Udata = scipy.stats.ortho_group.rvs(dim=dim_basis_inputs, size=1)
        Udata_jnp = jnp.asarray(Udata)

        ### Define correlation spectrum for data gen.
        Svals_data = np.ones(dim_basis_inputs)
        ...
        def Fourier_model_data(params, inputs):
            return Fourier_model_constructor(inputs, params, Udata_jnp, Svals_data_jnp, Vdata_tensors_jnp)
