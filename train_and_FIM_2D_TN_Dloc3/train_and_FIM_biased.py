"""Script for training partially tensorized biased models with both high and low ED.
...
- The data-generating function is f_theta0(x), the full model evaluated at a reference params0, whose
  V-tensor-train is built via generate_V_orthogonal_to_params to be orthogonal to iota(params0) up to
  'cutoff': the cutoff model's truncated spectrum then exactly reproduces f_theta0(x), so the model is biased.
