"""Script for aggregating the per-model-draw results of the Dloc3 biased/unbiased/slightly-biased
training scripts into consolidated pickle files for plotting.

- Does NOT train anything: it loads the pickled dict_results...pkl (and, for the
  slightly-biased case, model_biases...pkl) files saved once per (decay_exp or eps, model draw)
  combination by train_and_FIM_biased.py, train_and_FIM_UNbiased.py and
  train_and_FIM_SLIGHTbiased.py.
- For each scanned decay_exp (full-biased / unbiased case) or eps=delta_data value
  (slightly-biased case), it collects across model draws: Delta_{f-c}MSE_min statistics,
  MSE_full/cut statistics, normalized FIM spectra, normalized effective dimension, and (for the
  slightly-biased case) unitary_distance/matrix_distance/model_bias_full/model_bias_cut.
- Key swept variables: decay_exp_vec, eps_vec.
- Basic specs used to locate/match the right result files: no_of_features, max_frequency,
  no_params, dim_basis_single_param, cutoff, batch_size, learning_rate.
- Writes three consolidated pickle files, one per bias regime, for the companion plotting
  notebook: extracted_results_SLIGHTbiased*.pkl, extracted_results_FULLbiased*.pkl,
  extracted_results_UNbiased*.pkl.
"""
...
for nep in range(len(eps_vec)):
    ...
    for filename in listallfiles:
        ...
        dict_d['unitary_distance'] = []
        dict_d['matrix_distance'] = []
    ...
    for filename in listallfiles:
        ...
        dict_d['unitary_distance'].append(dict_biases['unitary_distance'])
        dict_d['matrix_distance'].append(dict_biases['matrix_distance'])
...
name_end = ('_Nfeatures' + name_no_features + '_MaxFreq' + name_max_freq + '_Nparams' + name_no_params + 
            '_DimBasisParam' + name_dim_basis_param + '_' + name_data_gen_fullbiased + '_batch' + batch_size_name + 
            '_lr' + learning_rate_name + '_cutoff' + cutoff_name + '_decayexp')
filename0 = 'dict_results' + name_end
listallfiles = [f for f in os.listdir(results_folder) if (f.startswith(filename0))]
...
name_end = ('_Nfeatures' + name_no_features + '_MaxFreq' + name_max_freq + '_Nparams' + name_no_params + 
            '_DimBasisParam' + name_dim_basis_param + '_' + name_data_gen_unbiased + '_batch' + batch_size_name + 
            '_lr' + learning_rate_name + '_cutoff' + cutoff_name + '_decayexp')
filename0 = 'dict_results' + name_end
listallfiles = [f for f in os.listdir(results_folder) if (f.startswith(filename0))]
...
name_end_extr = ('_Nfeatures' + name_no_features + '_MaxFreq' + name_max_freq + '_Nparams' + name_no_params + 
                 '_DimBasisParam' + name_dim_basis_param + '_cutoff' + cutoff_name)
