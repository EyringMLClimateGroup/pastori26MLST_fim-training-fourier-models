"""Scaling of training performance and effective dimension with Nparams at fixed Dloc=3 (UNbiased data generation).
...
For each of `no_rand_model_tests` random draws of (U, V) at a given Nparams value, this script
...
Other key control parameters (see 'Experiments specs' section below):
    no_rand_model_tests -- number of random (U, V, data-gen) model draws per Nparams value
