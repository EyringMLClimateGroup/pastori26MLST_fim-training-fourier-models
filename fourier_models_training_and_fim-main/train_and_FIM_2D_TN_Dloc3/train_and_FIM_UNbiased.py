"""Script for training partially tensorized unbiased models with both high and low ED.
...
- The data-generating function uses its own, independently and randomly drawn U/V structure constants
  (unrelated to the U/V of the trained full/cutoff models), so it is not encompassed by either model's
  expressible function space at any reference parameter configuration: the model is unbiased.
- For each random model/data-gen draw, the normalized FIM and effective dimension (via eff_dim_liminf) are
  computed for both the full and cutoff model, both are trained by minimizing MSE, and the resulting
  Delta_{f-c}MSE_min = MSE_full_min - MSE_cut_min is saved (2 input features, no_of_features=2).

Key control parameters (see 'Experiments specs' section below):
- no_of_features: number of input features (=2 in this folder).
- no_params: number of model parameters.
- dim_basis_single_param: local per-parameter basis dimension (Dloc, =3 here: 1, cos(theta), sin(theta)).
- bond_dim: bond dimension of the MPS representing V.
- max_frequency: maximal Fourier frequency per input feature.
- cutoff: index after which the cutoff model's correlation spectrum starts decaying.
- decay_exp_vec: vector of decay exponents looped over for the cutoff spectrum.
- no_rand_model_tests: number of random model/data-generating-function draws.
- no_train_tests: number of training runs (random parameter initializations) per model.
...
no_validation_data_per_feature = 15
...
#decay_exp_vec = [0.05, 0.07, 0.1, 0.2]
#decay_exp_name_vec = ['0p05', '0p07', '0p1', '0p2']
decay_exp_vec = [0.2]
decay_exp_name_vec = ['0p2']
#decay_exp_vec = [0.333, 0.5, 0.7]
#decay_exp_name_vec = ['0p333', '0p5', '0p7']
#decay_exp_vec = [1.0, 2.0, 3.33]
#decay_exp_name_vec = ['1p0', '2p0', '3p33']
