"""Script for training fully tensorized biased models with both high and low ED.
...
Biased model construction:
- The full and cutoff models share the same randomly-drawn MPO+TTN isometric mapping ('U')
  and the same tensor-train ('V', V_tensors_jnp), built via
  tensorized_model_constructor_functions.generate_V_orthogonal_to_params so that its rows are
  exactly orthogonal (up to index cutoff) to the local basis vectors iota(theta0) at a
  reference parameter draw params0.
- The data-generating function is the full model itself evaluated at params0; because V is
  orthogonal to iota(theta0) beyond cutoff, the cutoff model (correlation spectrum S decayed
  after cutoff, at rate decay_exp, i.e. Svals_cut vs. flat Svals_full) reproduces this
  data-generating function exactly, so both models are biased towards it.
...
- Save Delta_{f-c}MSE_min = MSE_full_min - MSE_cut_min (expected positive here: lower ED
  trains better for biased data) together with training curves and FIM spectra to a pickle
  file.
...
for nm in range(no_rand_model_tests):
    ...
    V_tensors_jnp = TNmodel_fns.generate_V_orthogonal_to_params(params0_jnp, cutoff, dim_corr_space, no_params, dim_basis_single_param, 
    ...
    for j in range(no_batches):
        ...
        out_batch = Fourier_model_full(params0, in_batch)
