"""Compute the scaling of the normalized effective dimension (ED, Eq. 12) with the number of
trainable parameters M (`no_params`) in a partially tensorized model, at fixed tensor-train bond
dimension and fixed local per-parameter basis dimension Dloc.

For each value of M in `no_params_vec` (the swept control parameter), the script draws
...
- `no_matrix_realiz`: number of random Gamma-equivalent model draws (random S/V realizations) per M value.
...
# Vector of no. of parameters [6, 10, 15, 20, 30, 40, 60, 80, 100, 130, 160, 200, 250, 300]
no_params_vec = [6, 10, 15, 20, 30, 40, 60, 80, 100]
#no_params_vec = [130, 160, 200]
#no_params_vec = [250, 300]
...
for no_params in no_params_vec:
    ...
    name_end = ('_BondDim' + name_bond_dim + '_DimIn' + name_dim_in + '_DimLocPar' + name_dim_par_loc + 
                ...
                '_NsamplesV' + no_V_samples_name + '_Nparams' + name_no_params)
    ...
    for nm in range(no_matrix_realiz):
        ...
        for ns in range(no_samples):
            ...
            no_params_all_2.append(no_params)
