...
- The full and cutoff models share the same U and V-tensor-train (V is built via
  generate_V_orthogonal_to_params so that both models reproduce the data-generating
  function exactly at the reference parameters params0, i.e. the model is "biased");
  they only differ in their correlation spectrum Svals_full vs. the truncated Svals_cut.
