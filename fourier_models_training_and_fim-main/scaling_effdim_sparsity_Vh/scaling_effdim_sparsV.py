"""Scaling of the (normalized) effective dimension (ED) with the sparsity index `s`
of V, the parameter-side singular-vector matrix in the SVD Gamma = U @ diag(S) @ V^T
of the structure constants, in a non-tensorized model. V is drawn as a block-sparse
random orthogonal matrix via `general_random_sparse_orthogonal` (see
`useful_functions/ortho_matrices_functions.py`), where the sparsity index interpolates
between a dense random orthogonal matrix (s=1) and a pure permutation matrix
(s=dim_basis_params).

For each sparsity index in `sparsity_vec`, draws `no_matrix_realiz` random
block-sparse orthogonal realizations of V (fixed correlation spectrum S with decay
exponent `dec_exp`), evaluates the normalized FIM at `no_samples` random parameter
draws per realization, and computes: the correlation-spectrum purity tr(S^4)
(constant across realizations here, since S is fixed) and the normalized effective
dimension (Eq. 12 of the paper) via the Monte-Carlo estimator `eff_dim_liminf`.
Results (sparsity index, tr(S^4), FIM purity, normalized ED) are pickled to
`results_folder` for later plotting.

Other key control parameters set in the "Experiments specs" section (the sparsity
index itself being the swept variable, via sparsity_vec):
...
- no_matrix_realiz: number of random block-sparse orthogonal realizations of V drawn
  per sparsity index.
- dec_exp: decay exponent of the correlation spectrum S (fixed across the sparsity
  scan).
...
# Vector of sparsity indices
log10_sparsity_vec = np.linspace(np.log10(2.0), np.log10(0.5 * dim_basis_params), num=16, endpoint=True)
sparsity_vec0 = np.int32(10.0 ** log10_sparsity_vec)
sparsity_vec = sparsity_vec0[0:4]
#sparsity_vec = sparsity_vec0[4:8]
#sparsity_vec = sparsity_vec0[8:12]
#sparsity_vec = sparsity_vec0[12:16]
...
for isp in range(len(sparsity_vec)):
    sparsity = int(sparsity_vec[isp])
    sparsity_name = str(sparsity)
    ...
    for nm in range(no_matrix_realiz):
        Vfull = ortho_fns.general_random_sparse_orthogonal(dim_basis_params, sparsity)
        Vh = Vfull[0:dim_in, :]
        ...
        for ns in range(no_samples):
            ...
            sparsityV_all_2.append(sparsity)
