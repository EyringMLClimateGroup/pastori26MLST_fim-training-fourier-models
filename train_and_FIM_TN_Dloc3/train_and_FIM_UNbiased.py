...
- The data-generating function uses its own independently-drawn U and V-tensor-train
  (Udata, Vdata_tensors_jnp: a random orthogonal matrix and a random right-orthogonalized
  tensor train, unrelated to the full/cut models' own U, V_tensors_jnp), so it is not
  encompassed by either model ("unbiased"); the full and cutoff models in turn share the
  same U/V and differ only in their correlation spectrum Svals_full vs. truncated Svals_cut.
- For both the full and cutoff model, computes the normalized FIM / effective dimension
  (via eff_dim_liminf) and trains by MSE minimization over no_train_tests random
  initializations, saving delta_minMSE_full_m_cut = MSE_full_min - MSE_cut_min per model draw.
- Key control params (see 'Experiments specs' below): no_of_features, no_params,
  dim_basis_single_param, bond_dim, max_frequency, cutoff, decay_exp_vec,
