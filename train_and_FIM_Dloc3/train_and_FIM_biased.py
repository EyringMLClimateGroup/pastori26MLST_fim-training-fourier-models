"""Script for training partially non-tensorized biased models with both high and low ED.
...
- Bias construction: a reference params0 is drawn and iota(theta0) (basis_funcs_th0) is computed;
  ortho_fns.gram_schmidt_ortho is used to build the model's V matrix (dense analogue of
  generate_V_orthogonal_to_params) so that its last dim_basis_inputs-cutoff columns are exactly
  orthogonal to iota(theta0). Full (coeffs_vectors_full, spectrum S) and cutoff
  (coeffs_vectors_cut, spectrum S with S[cutoff:] exponentially decayed via decay_exp) models
  share this U, V and agree on S[0:cutoff], so both reproduce the same f_theta0(x), which is used
  as the training data-generating function -> both models are exactly biased toward that data.
- For both the full and cutoff model, the normalized FIM is sampled over random parameters and
  ...
  Delta_{f-c}MSE_min = MSE_full_min - MSE_cut_min (expected positive here, since low ED helps a
  biased model) is saved to disk together with MSE and FIM-spectrum statistics.
...
for ndec in range(len(decay_exp_vec)):
    ...
    for nm in range(no_rand_model_tests):
        ...
        name_end = ('_Nfeatures' + name_no_features + '_MaxFreq' + name_max_freq + '_Nparams' + name_no_params + 
                    '_DimBasisParam' + name_dim_basis_param + '_' + name_data_gen + '_batch' + batch_size_name + 
