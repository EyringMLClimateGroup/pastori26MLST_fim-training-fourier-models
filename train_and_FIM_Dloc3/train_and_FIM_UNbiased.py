"""Script for training non-tensorized unbiased models with both high and low ED.
...
- Unbiased construction: the model's U, S, V are drawn from one random SVD (coeffs_vectors_0),
  giving full (spectrum S) and cutoff (spectrum S with S[cutoff:] exponentially decayed via
  decay_exp) models that share U, V. The training data-generating function instead uses an
  independently-drawn random coefficient matrix coeffs_vectors_data, unrelated to the models' own
  U/V, so the data is not (in general) in either model's expressible function space -> unbiased.
- For both the full and cutoff model, the normalized FIM is sampled over random parameters and
  ...
  Delta_{f-c}MSE_min = MSE_full_min - MSE_cut_min (expected negative here, since high ED helps an
  unbiased model) is saved to disk together with MSE and FIM-spectrum statistics.
...
for ndec in range(len(decay_exp_vec)):
    ...
    for nm in range(no_rand_model_tests):
        ...
        coeffs_vectors_data = 2.0 * np.random.rand(dim_basis_inputs, dim_basis_params) - 1.0
